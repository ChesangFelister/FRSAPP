...
export default function PhotoLightbox({ urls, startIndex, open, onOpenChange }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange} modal>
      <DialogContent
        className="max-w-[95vw] w-[95vw] h-[90vh] p-0 bg-background border-border overflow-hidden flex flex-col"
        onOpenAutoFocus={(e) => {
          e.preventDefault();
      ...
      >
        <div className="flex items-center justify-between px-4 py-2 border-b border-border bg-card z-10">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-xs text-muted-foreground tabular-nums w-12 text-center" aria-hidden="true">
              {Math.round(zoom * 100)}%
            </span>
            <Button
              variant="ghost"
        ...
        <div
          ...
          tabIndex={-1}
          role="region"
          aria-label="Image viewer. Use arrow keys to navigate, plus and minus to zoom, zero to reset."
        ...
        {urls.length > 1 && (
          <div className="flex gap-2 overflow-x-auto px-4 py-2 border-t border-border bg-card" role="tablist" aria-label="Photo thumbnails">
            {urls.map((u, i) => (
              <button
                key={u + i}
                ...
                role="tab"
                aria-label={`Go to photo ${i + 1} of ${urls.length}`}
